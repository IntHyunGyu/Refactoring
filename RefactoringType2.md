...
자바에서 추상화는 아직 미완성된 클래스나 메소드를 선언하여 구현을 더 쉽게 하는것? 이다.  추상화에 대해 학교 수업에서 간단한게 이런게 있다 정도로만 배우고 넘어갔기 때문에 이번 계기를 통해 명확히 알게 되었다.

**추상화**를 사용 하여 얻을수 있는 장점

- [**중복코드**]([https://ko.wikipedia.org/wiki/%EC%A4%91%EB%B3%B5_%EC%BD%94%EB%93%9C](https://ko.wikipedia.org/wiki/중복_코드))를 줄일수 있다.
...
* 효과 정보 Vo
  ...
  * **불필요한 변수**들	
    ...
    * 각각의 효과들은 결과를 처리할때 location만 계산이 필요한 경우, now만 필요한 경우, size만 필요한 경우 등 여러가지 케이스가 있다.
...
* 효과 정보 Vo
  ...
  * 공통 변수
    ...
    * 각 효과들이 공통적으로 사용하는 변수만 묶어 추상 클래스로 만들었다.
...
* 효과 출력
  ...
  * 여러 기능에 메소드
    ...
    * 1번에 효과 출력 메소드에서 언급한 것 처럼 **메소드**는 **하나의 기능**만을 가지고 있는것이 적절하다. 추상화를 적용하여 결과 처리 메소드를 하나로 처리 하였지만 이 코드 또한 효과 구분과 출력을 동시에 하고 있다.
